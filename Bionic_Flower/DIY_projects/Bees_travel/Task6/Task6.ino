/*Bee's travel, Task 6 : The flower lights in random color. 
The color sensor detects a color and if the path has been follow good .
If it is the case, the flower opens fully. 
If it is not, the flower turn off and stay closed .*/
...
boolean BLUE = false;
// PATH = true if the sensor is on the path, otherwise false
boolean PATH = false;
// END = true if the sensor is on the end of the path, otherwise false
boolean END = false;
...
// This value determines the threshold score value to win (70%)
int win_threshold=70;
...
{ 
  ...
  else if ((red_value> Path_R-range) && (red_value<Path_R+range)&& (green_value> Path_G-range) && (green_value< Path_G+range)&&(blue_value> Path_B-range) && (blue_value< Blue_B+range))
  {
    PATH= true; 
  }
  // Test if the sensor is on the end of the path , if it is the case, END = true
  else if ((red_value> End_R -range) && (red_value<End_R+range)&& (green_value> End_G-range) && (green_value< End_G+range)&&(blue_value> End_B-range) && (blue_value< End_B+range))
  {
    END= true; 
  }
...
void setup() 
{
 ...
 Serial.begin(9600);
 Serial.println("Bee's travel scenario : Task 6");
...
void loop() 
{  
  //The perfect_score value represents the maximum score that can be obtained (100%)
  int perfect_score=0;
  // The score value is the score we got when the path is followed 
  int score=0;
  //The RUN boolean value becomes true when a color has been detected until the end of the path has been detected
  boolean RUN =false;
  ...
  { 
      ...
      {
        Serial.println("#######");
        //The end of the path is detected
        RUN=false;
      }
      else 
      {
  ...
  //Calculating of the final score
  int final_score = score *100 / (perfect_score-1);
  Serial.print("Score finale: ");
  ...
  //Win
  if((good_color == true) && (final_score >=win_threshold ))
  { 
    Serial.println("Win");
