...
{
  int maximum=0;
  for ( int i=0;i<Number_player;i=i+1)
  {
    if(L[i]>L[maximum])
    {
      maximum=i;
    }
    else if(L[i]==L[maximum])
    {
		//0;1
      int rd=random(1);
      if(rd==1)
...
//LOOP: function that calculates the points for the touch sensor,the light sensor and the color sensor. Then,the function display for each player, their number and calculates their score. And the tail of the fish moves according to the score thanks to the Score_and_speed()function. The winner is the one with the highest score. You can restart the game.
void loop()
{
  while (Restart==true)
  { 
    ...
    for (int i=1;i<=Number_player;i=i+1)
   {
    ...
    for (int i=1;i<=Number_player;i=i+1)
    {
        ...
        Serial.println(i);
        Serial.println(Table_Score[i-1]);
        delay(200);
    }

   int Winner=max_score(Table_Score);
